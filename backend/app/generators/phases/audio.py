...
FAILED_AUDIO_URL = "failed to generate audio"
...
class AudioGenerator(Generator):
    ...
    async def generate(self) -> AsyncIterable[ArkChatResponse]:
        ...
        if self.mode == Mode.REGENERATION:
            ...
            for ri in audios_json:
                ...
                if audio.url and audio.url != FAILED_AUDIO_URL:
    ...
    async def _generate_audio(self, prompt: str, tone: str, index: int) -> str:
        try:
            ...
            req_id = get_reqid() or f"local-{uuid.uuid4()}"
            tos_object_key = f"{req_id}/{Phase.AUDIO.value}/{index}.mp3"
