...
- 每个分镜的首帧描述要简洁明了，字数不超过 200 字。
- 每个分镜的描述中必须包含场景信息。
- 每个首帧描述必须写明景别和画面占比。历史知识类默认使用远景、中远景或中景，让历史场景、建筑、文献、地图、空间纵深等信息占画面主要比例。
- 除非分镜明确是人物介绍或关键情绪转折，不要使用近景、大头照、半身肖像或居中海报式人物构图。
- 默认构图比例：背景/场景/文献/地图等信息元素占 60%-80%，角色人物占 15%-35%；纯地图、文献、时间线或建筑分镜中角色可占 0%-15%。
- 角色必须清晰可辨认，但应融入场景，可位于画面侧边、中景或远景位置，不要遮挡或压过背景参考图的主体氛围。
- 每个分镜的「角色」字段必须枚举画面中实际出现的视觉人物或视觉主体，且名称必须与「RoleDescription」中的角色名称保持一致。
- 如果 StoryBoard 的角色是“旁白”，但画面或台词提到了具体历史人物，则「角色」字段必须写这些具体历史人物，不要只写“旁白”。
- 只有当画面确实没有任何已定义历史人物或视觉主体时，才允许「角色」字段写“旁白”。
- 首帧描述中出现的已定义历史人物必须同步出现在「角色」字段中。
- 分镜数量需要和「StoryBoard」中的分镜数量严格保持一致。
...
分镜1：
角色：历史人物A，历史人物B
首帧描述：历史知识类动画插画，3D渲染，中远景构图，历史场景和文献道具占画面约70%，历史人物A和历史人物B位于画面侧边或中景位置，占画面约25%，人物清晰可辨认但不压过背景氛围。

分镜2：
角色：旁白
首帧描述：历史知识类动画插画，3D渲染，远景或俯视构图，没有已定义历史人物出镜，地图、文献、建筑或时间线信息占画面主体。
"""
)
    ...
    if content_mode == "history_knowledge":
        return ArkMessage(
            ...
            content=f"{HISTORY_KNOWLEDGE_FIRST_FRAME_DESCRIPTION_SYSTEM_PROMPT.content}\n\n"
                    ...
                    "- 背景参考图应主导整体色调、空间比例、时代氛围和场景结构；角色只作为场景中的人物或主体出现，不要默认占据画面中心和大面积比例。"
        )
