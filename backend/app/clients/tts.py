...
class TTSClient:
    ...
    async def receive_audio_data(self) -> ResponseEvent:
        ...
        while True:
            ...
            if result.error_code is not None:
                raise ProtocolError(f"TTS server error {result.error_code}: {result.payload_msg}")
    ...
    async def _connect(self, headers):
        ...
        return await websockets.connect(TTS_BASE_URL, **{header_arg: headers})

    def _safe_headers_for_log(self, headers):
        safe_headers = dict(headers)
        for key in ["X-Api-Access-Key", "X-Api-App-Key"]:
            if safe_headers.get(key):
                safe_headers[key] = "***"
        return safe_headers
    ...
    def _validate_config(self):
        ...
        if not TTS_ACCESS_KEY:
            missing.append("TTS_ACCESS_KEY or TTS_ACCESS_TOKEN")
        if not TTS_APP_KEY:
            missing.append("TTS_APP_KEY or TTS_APP_ID")
        if not TTS_BASE_URL:
...
def parse_response(res) -> ResponseEvent:
    ...
    if MsgType.bits_to_type(res[1] & 0xF0) == MsgType.ERROR:
        result.error_code = int.from_bytes(res[ptr: ptr + TTS_INT_SIZE], "big", signed=False)
        ptr += TTS_INT_SIZE
        result.payload_size = int.from_bytes(res[ptr: ptr + TTS_INT_SIZE], "big", signed=True)
        ptr += TTS_INT_SIZE
        payload_msg = res[ptr: ptr + result.payload_size]
