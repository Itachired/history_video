...
const internalAutoNextPhaseMap: Partial<Record<VideoGeneratorTaskPhase, VideoGeneratorTaskPhase>> = {
...
const RenderedMessagesProvider = (props: PropsWithChildren<Props>) => {
  ...
  const isReplayingMessagesRef = useRef<boolean>(false);
  const internalAutoProceedingRef = useRef<Record<string, boolean>>({});
  ...
  const shouldAutoProceedInsideFlowStep = (phase: string) => {
    const nextPhase = internalAutoNextPhaseMap[phase as VideoGeneratorTaskPhase];
    if (!nextPhase) {
      return false;
    }
    const nextPhaseConfig = PHASE_MAP[nextPhase];
    if (!nextPhaseConfig) {
      return false;
    }
    return !userConfirmMessageRef.current?.[nextPhaseConfig.userConfirmationDataKey];
  };

  const autoProceedInsideFlowStep = (phase: string) => {
    if (isReplayingMessagesRef.current || !shouldAutoProceedInsideFlowStep(phase)) {
      return false;
    }
    if (internalAutoProceedingRef.current[phase]) {
      return true;
    }
    internalAutoProceedingRef.current[phase] = true;
  ...
  const onMessageFinish = (newMessage: VideoGeneratorBotMessage, phase: string) => {
    ...
    // 产品大步骤内部自动推进；大步骤完成后停住，等待用户确认。
    if (
      checkStatus !== RunningPhaseStatus.ContentError &&
      autoProceedInsideFlowStep(phase)
    ) {
      return;
    }
  ...
  const resetMessages = () => {
    ...
    internalAutoProceedingRef.current = {};
    // update
    updateAutoNext(false);
    updateRunningPhaseStatus(RunningPhaseStatus.Ready);
    // 媒体图片关联信息
  ...
  useEffect(() => {
    ...
    if (initMessages.current) {
      isReplayingMessagesRef.current = true;
      const newRenderedMessages = generateRenderedMessages(messages);
      isReplayingMessagesRef.current = false;
  ...
  useEffect(() => {
    if (
      ...
      !storePhase ||
      !autoProceedInsideFlowStep(storePhase)
    ) {
      return;
    }
  }, [runningPhaseStatus, storePhase, userConfirmData]);
